...
resource "azurerm_virtual_machine_extension" "azure-monitor-agent" {
  ...
  for_each = {
    ...
    "AzureMonitorLinuxAgent" = {virtual_machine_id = "azurerm_linux_virtual_machine.myLinuxVm1.id", version = "1.10.0.0"}
  }
...
resource "azurerm_virtual_machine_extension" "azure-dependency-agent" {
  ...
  #type as key
  for_each = {
    "DependencyAgentWindows" = {virtual_machine_id = "azurerm_windows_virtual_machine.myWindowsVm1.id", version = "9.10"}
    "DependencyAgentLinux" = {virtual_machine_id = "azurerm_linux_virtual_machine.myLinuxVm1.id", version = "9.5"}
  }
