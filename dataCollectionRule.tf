...
resource "azurerm_monitor_data_collection_rule_association" "dcra" {
  ...
  for_each = {
        "windowsVM-dcra-" = {machine_id = "${data.azurerm_virtual_machine.windowsVM.id}", desc = "Windows VM data collection rule association"}
        "linuxVM-dcra" = {machine_id = "${data.azurerm_virtual_machine.linuxVM.id}", desc = "Linux VM data collection rule association"}
  }
  name                    = each.key
